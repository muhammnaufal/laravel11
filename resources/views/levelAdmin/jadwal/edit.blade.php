                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" name="id_matkul" id="exampleFormControlSelect1">
                                @foreach ($matakuliah as $data_matakuliah)
                                    <option value="{{ $data_matakuliah->id }}"
                                        {{ $data_matakuliah->id == $jadwal->id_matkul ? 'selected' : '' }}>
                                        {{ $data_matakuliah->nama_matakuliah }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                            ...
                            <select class="form-control" name="id_kelas" id="exampleFormControlSelect1">
                                @foreach ($kelas as $data_kelas)
                                    <option value="{{ $data_kelas->id }}"
                                        {{ $data_kelas->id == $jadwal->id_kelas ? 'selected' : '' }}>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" name="id_dosen" id="exampleFormControlSelect1">
                                @foreach ($dosen as $data_dosen)
                                    <option value="{{ $data_dosen->user_id }}"
                                        {{ $data_dosen->id == $jadwal->id_dosen ? 'selected' : '' }}>
                                        {{ $data_dosen->nama_dosen }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
